...
//
// Fail-closed on misconfig: in a production build with the var unset (or set
// empty - hence `||`, not `??`), `inngestRunUrl` returns null and the UI
// renders a non-link run id instead of a link to the operator's own
// localhost. Only dev builds default to the docker dev server.
//
...
const RAW_BASE = process.env.NEXT_PUBLIC_INNGEST_URL || ''
const INNGEST_BASE =
  RAW_BASE ||
