...
Schema lives in `apps/api/alembic/`. The first migration (`0001_create_extensions`) installs the `vector` and `citext` extensions; tables land in S1-06+.

```bash
make db-upgrade                  # alembic upgrade head
make db-downgrade                # alembic downgrade -1
make db-reset                    # downgrade to base, then upgrade to head
make db-revision m="add clients" # create a new autogenerated revision
```

All commands read `DATABASE_URL` from your environment (`.env` is loaded automatically). The async session and Alembic `env.py` rewrite the URL to `postgresql+asyncpg://` internally - keep `.env` in the canonical `postgresql://` form so `psql`, `verify_pgvector.py`, and the Neon CLI all keep working.
